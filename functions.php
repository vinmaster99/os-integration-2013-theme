...
function kriesi_pagination_text($pages = '', $range = 2)
{
	 $content = '';
     $showitems = ($range * 2)+1;  
     global $paged;
     ...
     if(1 != $pages)
     {
         ...
         for ($i=1; $i <= $pages; $i++)
         {
             if (1 != $pages &&( !($i >= $paged+$range+1 || $i <= $paged-$range-1) || $pages <= $showitems ))
             {
                 $content .= ($paged == $i)? "<li class='active'><a href=''>".$i."</a></li>":"<li><a href='".get_pagenum_link($i)."' class='' >".$i."</a></li>";
             }
         }
